...
class TinySSD(nn.Module):
    def __init__(self, num_classes=2, anchors_per_pixel=4):
        ...
        self.class_heads = nn.ModuleList([
            nn.Conv2d(
                channels[i],
                anchors_per_pixel * (num_classes + 1),
                3,
                padding=1,
            )
        ...
        self.box_heads = nn.ModuleList([
            nn.Conv2d(channels[i], anchors_per_pixel * 4, 3, padding=1)
            for i in range(1, len(channels))
        ])

    def forward(self, x):
        class_outputs, box_outputs = [], []
        for block, cls_head, box_head in zip(
            self.blocks, self.class_heads, self.box_heads
        ):
            x = block(x)
            ...
            class_outputs.append(
                cls_head(x)
                .permute(0, 2, 3, 1)
                .reshape(batch_size, -1, self.num_classes + 1)
            )
            box_outputs.append(
                box_head(x)
                .permute(0, 2, 3, 1)
